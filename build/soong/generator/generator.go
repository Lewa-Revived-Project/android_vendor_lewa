...
func init() {
	android.RegisterModuleType("lewa_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lewaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lewaExpandVariables(ctx, dep_file)
	...
	cmd := lewaExpandVariables(ctx, String(g.properties.Cmd))
